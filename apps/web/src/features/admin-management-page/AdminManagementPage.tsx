...
function AdminRow({ admin, onUpdated }: { admin: ManagedAdmin; onUpdated: () => void }) {
  ...
  return <>
    <tr className="border-t border-[#e7edf3]">
      ...
      <td className="p-4"><div className="flex flex-wrap gap-2"><Button disabled={mutation.isPending} onClick={() => { if (!admin.isActive || window.confirm(`Deactivate ${admin.name}? They will not be able to sign in until reactivated.`)) mutation.mutate({ isActive: !admin.isActive }); }} type="button" variant="secondary">{admin.isActive ? 'Deactivate' : 'Activate'}</Button><Button disabled={mutation.isPending} aria-expanded={editing} aria-label={`Edit ${admin.name}`} onClick={() => { if (editing) cancelEdit(); else setEditing(true); }} type="button" variant="secondary">Edit</Button></div>{mutation.isError ? <p className="mt-2 text-xs text-red-700" role="alert">{safeMessage(mutation.error)}</p> : null}</td>
    </tr>
