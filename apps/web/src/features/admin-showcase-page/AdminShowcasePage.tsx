...
export function AdminShowcasePage() {
  ...
  const deleteMutation = useMutation({
    ...
    onError: () => setActionError('Unable to delete this showcase article. Please try again.'),
  });

  const deleteShowcase = (id: string, title?: string) => {
    const articleTitle = title || articles.find((item) => item.id === id)?.title || 'this showcase article';
                        ...
                        <td className="py-4 text-right">
                          <div className="flex items-center justify-end gap-1">
                            ...
                            <button
                              ...
                              className="admin-row-action text-[#ef4147] hover:text-[#c53030]"
                              disabled={deleteMutation.isPending}
                              onClick={(e) => {
                                e.stopPropagation();
                                deleteShowcase(article.id, article.title);
                              }}
            ...
            <Card className="flex flex-col rounded-[26px] border-[#e1e8f0] bg-white p-6 shadow-[0_2px_4px_rgba(15,23,42,0.02)]">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {selectedArticle ? (
                    <button
                      aria-label={`Delete ${selectedArticle.title}`}
                      className="inline-flex items-center gap-1.5 rounded-xl border border-[#fed7d7] bg-[#fff5f5] px-3 py-1.5 text-xs font-bold text-[#ef4147] transition hover:bg-[#ffe3e3]"
                      disabled={deleteMutation.isPending}
                      onClick={() => deleteShowcase(selectedArticle.id, selectedArticle.title)}
                      type="button"
                    >
                      <svg className="size-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" />
                      </svg>
              ...
              {isEditingSelected ? (
                <div className="mt-4 space-y-4">
                  ...
                  <div>
                    <span className="block text-[12.5px] font-bold text-[#182238]">Related showcases (up to 3)</span>
                    <div className="mt-1 max-h-40 space-y-1 overflow-y-auto rounded-lg border border-[#dce5ef] p-2">
                      {articles.filter((item) => item.id !== selectedId).map((item) => {
                        const checked = editRelatedIds.includes(item.id);
                        const atLimit = editRelatedIds.length >= 3 && !checked;
                        return (
                          <label className={`flex items-center gap-2 rounded px-2 py-1.5 text-[13px] ${atLimit ? 'text-[#a3adb8]' : 'text-[#182238]'}`} key={item.id}>
                            <input
                              checked={checked}
                              disabled={atLimit}
                              onChange={(e) =>
                                setEditRelatedIds((current) =>
                                  e.target.checked ? [...current, item.id].slice(0, 3) : current.filter((id) => id !== item.id),
                                )
                              }
                              type="checkbox"
                            />
                      ...
                      {articles.length <= 1 ? <p className="px-2 py-1.5 text-[13px] text-[#71839e]">No other showcases available yet.</p> : null}
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2 pt-2">
                    <Button
                      className="rounded-xl border border-[#fed7d7] bg-[#fff5f5] text-[13.5px] font-semibold text-[#ef4147] hover:bg-[#ffe3e3]"
                      disabled={deleteMutation.isPending}
                      onClick={() => deleteShowcase(selectedArticle.id, selectedArticle.title)}
                      type="button"
                      variant="secondary"
