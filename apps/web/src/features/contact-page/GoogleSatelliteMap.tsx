...
export function GoogleSatelliteMap({
  ...
  return (
    <Card className="flex flex-col overflow-hidden rounded-2xl border border-[#edf2f7] bg-white shadow-[0_4px_16px_rgba(15,23,42,0.04)] transition duration-200 hover:shadow-[0_8px_24px_rgba(15,23,42,0.08)]">
      {/* Balanced Google Maps Satellite Viewport (~12% smaller) */}
      <div className="relative h-[250px] w-full overflow-hidden bg-[#eaf2f6] sm:h-[275px]">
        <iframe
          allowFullScreen
        ...
        <div className="pointer-events-none absolute right-2.5 top-2.5 inline-flex items-center gap-1.5 rounded-full bg-white/95 px-2.5 py-1 text-[10.5px] font-bold text-[#3695B9] shadow-sm backdrop-blur">
      ...
      <div className="flex flex-1 flex-col justify-between p-4 sm:p-5">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 line-clamp-1 text-[12.5px] font-medium text-[#6b7280]">{address}</p>
          </div>

          {/* Action Button */}
          <a
            className="inline-flex shrink-0 items-center gap-1.5 rounded-full bg-[#3695B9] px-3.5 py-1.5 text-[12px] font-bold text-white shadow-[0_4px_10px_rgba(54,149,185,0.22)] transition hover:-translate-y-0.5 hover:bg-[#2c84a5]"
            href={directionsUrl}
        ...
        {(phone || hours) && (
          <div className="mt-3 flex flex-wrap items-center justify-between gap-x-5 gap-y-1 border-t border-[#edf2f7] pt-2.5 text-[11.5px]">
            {phone && (
              <div className="flex items-center gap-1.5">
