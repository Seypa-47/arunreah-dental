    ...
    <div className={`flex items-center ${compact ? 'gap-4' : 'gap-5'}`}>
      ...
      <div>
        ...
        {!compact && item.description !== item.value ? (
          <p className="text-[12px] font-medium leading-4 text-[#7d8792]">{item.description}</p>
        ) : null}
      </div>
    </div>
  );
  ...
  return (
    <section className="relative overflow-hidden bg-white">
      <div className="relative min-h-[560px]">
        ...
        <div className="relative mx-auto grid min-h-[560px] w-full max-w-[1280px] items-center gap-10 px-4 py-16 sm:px-6 lg:grid-cols-[1fr_410px] lg:px-0">
          <div className="max-w-[620px]">
            ...
            <p className="mt-3 max-w-[600px] text-[18px] font-medium leading-8 text-[#5f6974]">{hero.subtitle}</p>
          </div>
...
function ContactCards({ cards }: { cards: ContactPageContent['contactCards'] }) {
  return (
    <section className="bg-[#f1f7fa] py-[62px]">
      <div className="mx-auto grid w-full max-w-[1280px] gap-6 px-4 sm:grid-cols-2 sm:px-6 lg:grid-cols-4 lg:px-0">
        {cards.map((card) => (
          <Card className="rounded-2xl border-[#e4edf2] px-7 py-6 shadow-[0_8px_22px_rgba(15,23,42,0.04)]" key={card.label}>
...
const fieldClass =
  'min-h-[54px] w-full rounded-xl border border-[#d9e6ed] bg-[#f8fbfd] px-4 text-[14px] font-medium text-[#17364c] outline-none transition placeholder:text-[#a8b3bf] focus:border-[#3695b9] focus:bg-white focus:ring-2 focus:ring-[#d9f0f7]';
...
function ContactForm({ form }: { form: ContactPageContent['form'] }) {
  ...
  return (
    <section className="bg-[#f1f7fa] pb-[46px]">
      <Card className="mx-auto w-[calc(100%-32px)] max-w-[1280px] rounded-2xl border-[#e4edf2] px-6 py-10 shadow-[0_10px_28px_rgba(15,23,42,0.04)] sm:px-10 lg:px-12">
        ...
        <form className="mt-9 grid gap-x-6 gap-y-6" onSubmit={handleSubmit}>
          ...
          <div>
            ...
            <textarea
              ...
              value={message}
            />
            <p className="mt-2 text-right text-[12px] font-semibold text-[#b0bac5]" id="message-count">
...
function MapsSection({ maps }: { maps: ContactPageContent['maps'] }) {
  return (
    <section className="bg-[#f1f7fa] pb-[108px]">
      <div className="mx-auto grid w-full max-w-[1280px] gap-10 px-4 sm:px-6 lg:grid-cols-2 lg:px-0">
        {maps.map((map) => (
          <article className="relative overflow-hidden rounded-sm" key={map.imageAlt}>
            ...
            <span className="absolute left-1/2 top-[53%] inline-flex -translate-x-1/2 items-center gap-2 rounded-lg bg-white px-5 py-3 text-[13px] font-extrabold text-[#17364c] shadow-[0_10px_22px_rgba(15,23,42,0.18)]">
  ...
  return (
    <SiteLayout actions={{ appointmentLabel: 'Book Appointment', contactLabel: 'Contact Us' }} navigation={skeletonNavigation}>
      <main aria-busy="true" aria-label="Loading contact page" className="bg-[#f1f7fa]">
        ...
        <section className="mx-auto grid max-w-[1280px] gap-6 px-4 py-[62px] sm:px-6 lg:grid-cols-4 lg:px-0">
