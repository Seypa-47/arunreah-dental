import { createPageMediaSchema, pageMediaAdminQuerySchema, pageMediaPublicQuerySchema, successResponse, updatePageMediaSchema } from '@arunreah/shared';
...
export async function listPublic(c: Context<AppEnv>) { const q = parseRequestQuery(c, pageMediaPublicQuerySchema); applyPublicCmsCache(c); return c.json(successResponse(await service.listPublic(createDbClient(c.env.DB), q.placement, q.lang))); }
export async function listAdmin(c: Context<AppEnv>) { const { placement } = parseRequestQuery(c, pageMediaAdminQuerySchema); c.header('Cache-Control', 'private, no-store'); return c.json(successResponse(await service.listAdmin(createDbClient(c.env.DB), placement))); }
