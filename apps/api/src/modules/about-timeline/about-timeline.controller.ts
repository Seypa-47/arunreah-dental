...
export async function listPublic(c: Context<AppEnv>) { const query = parseRequestQuery(c, aboutTimelinePublicQuerySchema); applyPublicCmsCache(c); return c.json(successResponse(await service.listPublic(createDbClient(c.env.DB), query.lang))); }
