...
export async function listPublic(db: DatabaseClient, placement: PageMediaPlacement, lang: 'en' | 'km') { return { items: (await withPresentations(db, await repository.listPublic(db, placement))).map((item) => ({ id: item.id, imageKey: item.imageKey, imagePresentation: item.imagePresentation, title: lang === 'km' ? item.titleKm : item.titleEn, body: lang === 'km' ? item.bodyKm : item.bodyEn, badge: lang === 'km' ? item.badgeKm : item.badgeEn, benefits: lang === 'km' ? item.benefitsKm : item.benefitsEn, discount: lang === 'km' ? item.discountKm : item.discountEn, validUntil: item.validUntil, displayOrder: item.displayOrder })) }; }
