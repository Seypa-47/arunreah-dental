...
export async function listPublic(db: DatabaseClient, lang: 'en' | 'km') {
  return { items: (await repository.listPublic(db)).map((item) => ({ id: item.id, year: item.year, title: lang === 'km' ? item.titleKm : item.titleEn, body: lang === 'km' ? item.bodyKm : item.bodyEn, displayOrder: item.displayOrder })) };
}
